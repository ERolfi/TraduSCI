...
uint16 GetBitsMSB (uint8 **source, uint16 *bitpos, uint16 bits) {
        uint32 x32;
        uint8  *s;
        uint16 x;
...
int lzwmsb_Decompress (uint8 *source, uint8 *target, uint32 outsize) {
        uint8  *s, *t;
        uint16  BitPosition;
        uint32  BytesWritten;
        ...
        PrefixCode = GetBitsMSB (&s, &BitPosition, CodeLength);
        *t++ = PrefixCode; BytesWritten++;

        while (BytesWritten <= outsize) {
           ReadCode = GetBitsMSB (&s, &BitPosition, CodeLength);
           ThisCode = ReadCode;
