...
//TODO ci sono problemi in Windows 11 con MAX_PATH, il sistema può passare al programma path molto lunghi. trovare una soluzione!
// Global Variables:
...
void ShowLoopCell(unsigned char newloop, unsigned char newcell)
{
	...
	if (tloop)
	{
		...
		//if (curCell || (*tloop)->Mirrored())
		{
			...
			EnableMenuItem(menu, ID_IMPORTABMP, ((*tloop)->Mirrored() || !curShownImage ?MF_GRAYED :MF_ENABLED));
			EnableMenuItem(menu, ID_ESPORTABMP, ((*tloop)->Mirrored() || !curShownImage ?MF_GRAYED :MF_ENABLED));
